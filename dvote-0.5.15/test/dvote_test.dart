...
void hdWallet() {
  const MethodChannel channel = MethodChannel('dvote');

  setUp(() {
    channel.setMockMethodCallHandler((MethodCall methodCall) async {
      return '0xbf0177fac1aad0cba87e5c0f54742fae36d69fff';
    });
  });
  ...
  test('mnemonicToAddress', () async {
    expect(
        await mnemonicToAddress(
            'master lift organ scale accuse under head grace deny panic apart check',
            hdPath: "m/44'/60'/0'/0/0"),
        '0xbf0177fac1aad0cba87e5c0f54742fae36d69fff');
  });
}

void vocGateway() {
  final origin = "ipfs://Qmaisz6NMhDB51cCvNWa1GMS7LU1pAxdF4Ld6Ft9kZEP2a";
  final gwUri = "ws://gateway:2082/dvote";
  final publicKey =
      "02325f284f50fa52d53579c7873a480b351cc20f7780fa556929f5017283ad2449";

  test('fetchFile', () async {
    final res = await fetchFileString(ContentURI(origin), gwUri,
...
void dataModels() {
  test("Entity metadata model", () async {
    ...
    meta =
        """{"version":"1.0","languages":["fr","en"],"name":{"default":"My official entity","fr":"Bonjour","en":"Good morning"},"description":{"default":"The description of my entity goes here","fr":"Bonjour 123","en":"Good morning 123"}},"votingProcesses":{"active":[],"ended":[]},"newsFeed":{"default":"https://hipsterpixel.co/feed.json"},"media":{"avatar":"https://hipsterpixel.co/assets/favicons/apple-touch-icon.png","header":"https://images.unsplash.com/photo-1557518016-299b3b3c2e7f?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=500&q=80"},"actions":[{"type":"browser","name":{"default":"Sign up to The Entity"},"url":"https://cloudflare-ipfs.com/ipfs/QmZ56Z2kpG5QjJcWfhxFD4ac3DhfX21hrQ2gCTrWxzTAse","visible":"always"},{"type":"browser","name":{"default":"Sign up"},"register":true,"url":"https://registry.vocdoni.net/register.html?entityId=0x180dd5765d9f7ecef810b565a2e5bd14a3ccd536c442b3de74867df552855e85","visible":"https://registry.vocdoni.net/api/actions/status?action=register?action=register"}],"bootEntities":[],"fallbackBootNodeEntities":[],"trustedEntities":[],"censusServiceManagedEntities":[]}""";
    entity = parseEntityMetadata(meta);
    expect(entity.actions[0].type, "browser",
        reason: "The action type should be browser");
    expect(entity.actions[0].register, false,
        reason: "The action register should be false");
    expect(entity.actions[1].type, "browser",
        reason: "The action type should be browser");
    expect(entity.actions[1].register, true,
        reason: "The action register should be true");
  });
